...
namespace DelegatesLinQ.Homework
{
    ...
    public class LinqDataProcessor
    {
        ...
        public void BasicQueries()
        {
            ...
            var gpaQuery = _students.Where(s => s.GPA > 3.5);
            foreach (var student in gpaQuery)
            {
                Console.WriteLine($" {student.Name}: {student.GPA}");
            }
            
            Console.WriteLine("\n2. Students grouped by major:");
            var studentByMajor = _students.GroupBy(s => s.Major);
            foreach (var group in studentByMajor)
            {
                Console.WriteLine($"\nMajor: {group.Key}");
            ...
            var avgGpaPerMajor = _students.GroupBy(s => s.Major)
                .Select(g => new { Major = g.Key, AverageGPA = g.Average(s => s.GPA) });
            foreach (var item in avgGpaPerMajor)
            {
                Console.WriteLine($"- {item.Major}: {item.AverageGPA}");
            }
            
            Console.WriteLine("\n4. Students enrolled in 'Intro to Programming':");
            var studentInCourse = _students.Where(s => s.Courses.Any(c => c.Name == "Intro to Programming"))
                .Select(s => s.Name);
            foreach (var name in studentInCourse)
            {
            ...
            var sortedByEnrollment = _students
                ...
                .Select(s => new { s.Name, EnrollmentDate = s.EnrollmentDate.ToString("dd-MM-yyyy") });
            foreach (var student in sortedByEnrollment)
            {
                Console.WriteLine($"- {student.Name}: {student.EnrollmentDate}");
            }

        }

        // Challenge 1: Create custom extension methods
        public static void CustomExtensionMethods()
        {
            Console.WriteLine("=== CUSTOM EXTENSION METHODS ===");

            // TODO: Implement extension methods
            // 1. CreateAverageGPAByMajor() extension method
            static Dictionary<string, double> AverageGPAByMajor()
            {
            ...
            return new List<Student>
            {
                new Student
                {
                    ...
                    Courses = new List<Course>
                    {
                        new Course
                        {
                            Code = "CS101", Name = "Intro to Programming", Credits = 3, Grade = 3.7,
                            Semester = "Fall 2022", Instructor = "Dr. Smith"
                        },
                        new Course
                        {
                            Code = "MATH201", Name = "Calculus II", Credits = 4, Grade = 3.9, Semester = "Fall 2022",
                ...
                new Student
                {
                    ...
                    Courses = new List<Course>
                    {
                        new Course
                        {
                            Code = "MATH301", Name = "Linear Algebra", Credits = 3, Grade = 3.3,
                            Semester = "Spring 2023", Instructor = "Dr. Brown"
                        },
                        new Course
                        {
                            Code = "STAT101", Name = "Statistics", Credits = 3, Grade = 3.1, Semester = "Spring 2023",
                ...
                new Student
                {
                    ...
                    Courses = new List<Course>
                    {
                        new Course
                        {
                            Code = "CS102", Name = "Data Structures", Credits = 4, Grade = 4.0, Semester = "Fall 2023",
                            Instructor = "Dr. Smith"
                        },
                        new Course
                        {
                            Code = "CS201", Name = "Algorithms", Credits = 3, Grade = 3.8, Semester = "Fall 2023",
                            Instructor = "Prof. Lee"
                        }
                    }
                }
    ...
    public class LinqDataProcessorMain
    {
